...
const BannerCarousel: React.FC<BannerCarouselProps> = ({ imageUrls }) => {
  ...
  return (
    <div className="relative container1">
      <div className="w-[100%] relative sm:h-[300px] h-[250px]  rounded-b-lg mx-auto overflow-hidden rou">
        {images.map((image, index) => (
          <img
            key={index}
            src={image.src}
            alt={`Banner ${index + 1}`}
            className={`absolute top-0 left-0 w-[100%] mx-auto sm:h-[300px] opacity-85 h-[250px] transition-transform duration-500 ${
      ...
      <div className="flex justify-between items-center mt-2">
        <button
          onClick={prevSlide}
          className="absolute sm:-left-4 -left-2 top-1/2 transform -translate-y-1/2 text-bg_gray p-1 font-bold rounded-full"
        >
          <svg
        ...
        <button
          onClick={nextSlide}
          className="absolute -right-2 sm:-right-4 top-1/2 transform -translate-y-1/2  text-bg_gray p-1 font-bold rounded-full"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
